...
export function SettingsPage() {
  ...
  if (isError && !savedSettings) {
    return <><PageHeader title="Cài đặt hệ thống" description="Thiết lập thông tin, thời gian hoạt động và phương thức thanh toán của bãi đỗ xe." /><Card><CardContent className="grid justify-items-center gap-3 py-12 text-center"><p className="text-sm text-red-600">{errorMessage}</p><Button type="button" variant="secondary" onClick={() => refetch()}>Thử lại</Button></CardContent></Card></>;
  }

  if (isLoading || !form || !savedSettings) {
    ...
    <>
      ...
      <form className="grid gap-6" onSubmit={submit} noValidate>
          ...
          <CardContent className="grid gap-4 md:grid-cols-2">
            <div className="md:col-span-2">
              <Field label="Tên hệ thống" error={submitted ? errors.systemName : undefined}>
                <Input value={form.systemName} onChange={(event) => update("systemName", event.target.value)} maxLength={100} aria-invalid={submitted && !!errors.systemName} />
              </Field>
        ...
        <Card>
          ...
          <CardContent className="grid gap-4 md:grid-cols-2">
            <Field label="Giờ mở cửa" error={submitted ? errors.openingTime : undefined}><Input type="time" value={form.openingTime} onChange={(event) => update("openingTime", event.target.value)} /></Field>
            <Field label="Giờ đóng cửa" error={submitted ? errors.closingTime : undefined}><Input type="time" value={form.closingTime} onChange={(event) => update("closingTime", event.target.value)} /></Field>
        ...
        <Card>
          ...
          <CardContent className="grid gap-4 md:grid-cols-2">
            <Field label="Phương thức thanh toán">
              <Select value={form.paymentMode} onChange={(event) => update("paymentMode", event.target.value as UpdateSystemSettingsRequest["paymentMode"])}><option value="CASH">Tiền mặt</option><option value="CASHLESS">Không tiền mặt</option><option value="HYBRID">Kết hợp</option></Select>
            </Field>
            <Field label="Tự động khóa slot quá hạn">
              <Select value={form.autoBlockOverdueSlots ? "true" : "false"} onChange={(event) => update("autoBlockOverdueSlots", event.target.value === "true")}><option value="true">Bật</option><option value="false">Tắt</option></Select>
            </Field>
        ...
        <div className="sticky bottom-0 flex flex-wrap items-center justify-between gap-3 border-t border-border bg-white/95 px-5 py-4 shadow-[0_-4px_12px_rgba(15,23,42,0.05)] backdrop-blur">
          <p className="text-sm text-slate-500">{isDirty ? "Bạn có thay đổi chưa lưu." : `Cập nhật lần cuối: ${new Date(savedSettings.updatedAt).toLocaleString("vi-VN")}`}</p>
          <div className="flex gap-2">
            <Button type="button" variant="secondary" onClick={reset} disabled={!isDirty || updateMutation.isPending}><RotateCcw size={16} /> Hoàn tác</Button>
            <Button type="submit" disabled={!isDirty || updateMutation.isPending}><Save size={17} /> {updateMutation.isPending ? "Đang lưu..." : "Lưu cấu hình"}</Button>
          </div>
        </div>
      </form>
    </>
