...
export function SessionsPage() {
  ...
  const canManageExceptions = hasPermission(role, "exceptions:manage");
  ...
  return (
    <>
      ...
      <Card>
        <CardHeader title="Danh sách session" action={canManageExceptions && <Button variant="secondary" onClick={() => setException("LOST_TICKET")}><AlertTriangle size={17} /> Xử lý ngoại lệ</Button>} />
        <CardContent><DataTable data={data} columns={columns} /></CardContent>
      </Card>
      {canManageExceptions && <div className="mt-6 grid gap-3 md:grid-cols-5">
      ...
      {canManageExceptions && <ExceptionModal type={exception} onClose={() => setException(null)} />}
    </>
  );
}
