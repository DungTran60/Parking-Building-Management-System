**Project**: Hệ thống quản lý tòa nhà gửi xe 
...
**Primary Actor**: Parking Facility Manager, Parking Staff, Parking User / Driver, System Administrator 
...
### 7. Quản lý Bảng Giá (Pricing CRUD)

Quản lý bảng giá đỗ xe theo loại phương tiện và đơn vị thời gian (giờ / ngày / tháng).
...
```json
{
  ...
  "timeUnit": "HOURLY",
  "price": 5000,
  "description": "Giá gửi xe ô tô theo giờ",
  "active": true
}
```
(timeUnit có thể là: HOURLY | DAILY | MONTHLY)
Hệ thống sẽ trả về thông tin bảng giá vừa tạo kèm id.
Lưu ý: mỗi cặp (vehicleTypeId + timeUnit) phải là duy nhất.
...
Lấy chỉ bảng giá đang áp dụng (active = true):
Method: GET
URL: http://localhost:8080/api/pricing?active=true

---

**Lấy bảng giá theo ID:**
Method: GET
URL: http://localhost:8080/api/pricing/{id}

---

**Lấy bảng giá theo loại phương tiện:**
Method: GET
URL: http://localhost:8080/api/pricing/vehicle-type/{vehicleTypeId}

Ví dụ: http://localhost:8080/api/pricing/vehicle-type/car
Lọc chỉ lấy bảng giá đang áp dụng:
URL: http://localhost:8080/api/pricing/vehicle-type/car?active=true
...
```json
{
  ...
  "timeUnit": "HOURLY",
  "price": 6000,
  "description": "Cập nhật giá gửi xe ô tô theo giờ",
  "active": true
}
```
...
**Bật / Tắt trạng thái áp dụng bảng giá:**
Method: PATCH
URL: http://localhost:8080/api/pricing/{id}/toggle
(Không cần body. Hệ thống tự đảo trạng thái active hiện tại.)
...
**Phân quyền:**
- POST / PUT / PATCH: yêu cầu role ADMIN hoặc MANAGER
- DELETE: chỉ ADMIN
- GET: tất cả user đã đăng nhập

**Response mẫu (PricingResponseDto):**
```json
{
  "id": 1,
  "vehicleTypeId": "car",
  ...
  "description": "Giá gửi xe ô tô theo giờ",
  "active": true,
  "createdAt": "2026-07-01T22:00:00",
  "updatedAt": "2026-07-01T22:00:00"
}
```
